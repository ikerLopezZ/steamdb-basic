...
/**
 * Función para conectarse a Steam y obtener información de aplicaciones.
 */
const fetchAndStoreApps = () => {
  ...
  client.on("loggedOn", async () => {
    ...
    // AppIDs de los 100 juegos mejores valorados de Steam de 2023
    const appIDs = [
    ...
    client.getProductInfo(
      appIDs,
      [],
      async (err, apps, packages, unknownApps, unknownPackages) => {
        if (err) {
            ...
            if (!existingApp) {
              ...
              const app = new App({
                ...
                supportedLanguages: appInfo.supported_languages
                  ? Object.keys(appInfo.supported_languages)
                  : [],
              ...
              console.log(
                `Juego ${existingApp.name} ya existe en la base de datos`
              );
            }
          }
        }
